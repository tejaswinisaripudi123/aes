import javax.crypto.KeyGenerator;
import javax.crypto.SecretKey;

public class AES {

   private static int Nb, Nk, Nr;//no of blocks,no of words in key,no of rounds
   private static byte[][] w;//words
   static double t1,t2;//starting time,ending time

   private static int[] sbox = {  };//S BOX padding for substitution

   private static int[] inv_sbox = {  };//inverse sbox padding for substitution

   private static int Rcon[] = {  };//round constant

   public static byte[] keygeneration() throws Exception  {

       KeyGenerator generator = KeyGenerator.getInstance("AES");//to generate instance of aes to generate key
       generator.init(128);// to intialize the key of required size
       SecretKey key = generator.generateKey();//to generate secret key
       return key.getEncoded();/*Returns the key in its primary encoding format, or null if this key does not support encoding.*/

   }

   private static byte[] xor_func(byte[] a, byte[] b) {
       byte[] out = new byte[a.length];//output
       for (int i = 0; i < a.length; i++) {
           out[i] = (byte) (a[i] ^ b[i]);//
       }
       return out;//method for xor operation 

   }

   private static byte[][] generateSubkeys(byte[] key) {
       byte[][] tmp = new byte[Nb * (Nr + 1)][4];

       int i = 0;
       while (i < Nk) {

           tmp[i][0] = key[i * 4];
           tmp[i][1] = key[i * 4 + 1];
           tmp[i][2] = key[i * 4 + 2];
           tmp[i][3] = key[i * 4 + 3];
           i++;
       }
       i = Nk;
       while (i < Nb * (Nr + 1)) {
           byte[] temp = new byte[4];
           for(int k = 0;k<4;k++)
               temp[k] = tmp[i-1][k];
           if (i % Nk == 0) {
               temp = SubWord(rotateWord(temp));
               temp[0] = (byte) (temp[0] ^ (Rcon[i / Nk] & 0xff));
           } else if (Nk > 6 && i % Nk == 4) {
               temp = SubWord(temp);
           }
           tmp[i] = xor_func(tmp[i - Nk], temp);
           i++;
       }

       return tmp;//method used for key expansion
   }

   private static byte[] SubWord(byte[] in) {
       byte[] tmp = new byte[in.length];

       for (int i = 0; i < tmp.length; i++)
           tmp[i] = (byte) (sbox[in[i] & 0x000000ff] & 0xff);//byte to hex

       return tmp;// method to substitute nibble 
   }

   private static byte[] rotateWord(byte[] input) {
       byte[] tmp = new byte[input.length];
       tmp[0] = input[1];
       tmp[1] = input[2];
       tmp[2] = input[3];
       tmp[3] = input[0];

       return tmp; //to rotate the nibble
   }

   private static byte[][] AddRoundKey(byte[][] state, byte[][] w, int round) {

       byte[][] tmp = new byte[state.length][state[0].length];

       for (int c = 0; c < Nb; c++) {
           for (int l = 0; l < 4; l++)
               tmp[l][c] = (byte) (state[l][c] ^ w[round * Nb + c][l]);
       }

       return tmp;
   }   //to add the round key

   private static byte[][] SubBytes(byte[][] state) {

       byte[][] tmp = new byte[state.length][state[0].length];
       for (int row = 0; row < 4; row++)
           for (int col = 0; col < Nb; col++)
               tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);//substituting s box values by matching the current values in box

       return tmp;//method to substitute using sbox for encryption
   }
   private static byte[][] InvSubBytes(byte[][] state) {
       for (int row = 0; row < 4; row++)
           for (int col = 0; col < Nb; col++)
               state[row][col] = (byte)(inv_sbox[(state[row][col] & 0x000000ff)]&0xff);

       return state; //method to substitute using sbox for decryption
   }

   private static byte[][] ShiftRows(byte[][] state) {

       byte[] t = new byte[4];
       for (int r = 1; r < 4; r++) {
           for (int c = 0; c < Nb; c++)
               t[c] = state[r][(c + r) % Nb];
           for (int c = 0; c < Nb; c++)
               state[r][c] = t[c];
       }

       return state; //method to swap rows for encryption except first row
   }

   private static byte[][] InvShiftRows(byte[][] state) {
       byte[] t = new byte[4];
       for (int r = 1; r < 4; r++) {
           for (int c = 0; c < Nb; c++)
               t[(c + r)%Nb] = state[r][c];
           for (int c = 0; c < Nb; c++)
               state[r][c] = t[c];
       }
       return state; //method to swap  row for decryption

   }

   private static byte[][] InvMixColumns(byte[][] s){
       int[] sp = new int[4];
       byte b02 = (byte)0x0e, b03 = (byte)0x0b, b04 = (byte)0x0d, b05 = (byte)0x09;
       for (int c = 0; c < 4; c++) {
           sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ FFMul(b04,s[2][c])  ^ FFMul(b05,s[3][c]);
           sp[1] = FFMul(b05, s[0][c]) ^ FFMul(b02, s[1][c]) ^ FFMul(b03,s[2][c])  ^ FFMul(b04,s[3][c]);
           sp[2] = FFMul(b04, s[0][c]) ^ FFMul(b05, s[1][c]) ^ FFMul(b02,s[2][c])  ^ FFMul(b03,s[3][c]);
           sp[3] = FFMul(b03, s[0][c]) ^ FFMul(b04, s[1][c]) ^ FFMul(b05,s[2][c])  ^ FFMul(b02,s[3][c]);
           for (int i = 0; i < 4; i++) s[i][c] = (byte)(sp[i]);
       }

       return s; //to generate mix columns for decryption
   }

   private static byte[][] MixColumns(byte[][] s){
       int[] sp = new int[4];
       byte b02 = (byte)0x02, b03 = (byte)0x03;
       for (int c = 0; c < 4; c++) {
           sp[0] = FFMul(b02, s[0][c]) ^ FFMul(b03, s[1][c]) ^ s[2][c]  ^ s[3][c];
           sp[1] = s[0][c]  ^ FFMul(b02, s[1][c]) ^ FFMul(b03, s[2][c]) ^ s[3][c];
           sp[2] = s[0][c]  ^ s[1][c]  ^ FFMul(b02, s[2][c]) ^ FFMul(b03, s[3][c]);
           sp[3] = FFMul(b03, s[0][c]) ^ s[1][c]  ^ s[2][c]  ^ FFMul(b02, s[3][c]);
           for (int i = 0; i < 4; i++) s[i][c] = (byte)(sp[i]);
       }

       return s;//to generate mix columns for encryption
   }

   public static byte FFMul(byte a, byte b) {
       byte aa = a, bb = b, r = 0, t;
       while (aa != 0) {
           if ((aa & 1) != 0)
               r = (byte) (r ^ bb);
           t = (byte) (bb & 0x80);
           bb = (byte) (bb << 1);
           if (t != 0)
               bb = (byte) (bb ^ 0x1b);
           aa = (byte) ((aa & 0xff) >> 1);
       }
       return r; //field multiplication for mixed columns

   }

   public static byte[] encryptBloc(byte[] in) {//method used for encryp block
       byte[] tmp = new byte[in.length]; //creating a temp array to return the encrypted bytes



       byte[][] state = new byte[4][Nb]; // the state array


       for (int i = 0; i < in.length; i++)
           state[i / 4][i % 4] = in[i%4*4+i/4];

       state = AddRoundKey(state, w, 0);
       for (int round = 1; round < Nr; round++) { //rounds for encryption
           state = SubBytes(state); //calling method for s-box substitution
           state = ShiftRows(state);//calling method to shift the rows
           state = MixColumns(state);// method for complicated mix of columns
           state = AddRoundKey(state, w, round);// xor with expanded key

       }
       state = SubBytes(state);
       state = ShiftRows(state);// for the last round skipping mixcols
       state = AddRoundKey(state, w, Nr);

       for (int i = 0; i < tmp.length; i++)
           tmp[i%4*4+i/4] = state[i / 4][i%4];

       return tmp; //returning the encrypted bytes
   }

   public static byte[] decryptBloc(byte[] in) {//method used for decrypt block of data
       byte[] tmp = new byte[in.length];

       byte[][] state = new byte[4][Nb];// the state array


       for (int i = 0; i < in.length; i++)
           state[i / 4][i % 4] = in[i%4*4+i/4];

       state = AddRoundKey(state, w, Nr);
       for (int round = Nr-1; round >=1; round--) {
           state = InvSubBytes(state);//inverse s-box substitution

           state = InvShiftRows(state);//inverse shift rows
           state = AddRoundKey(state, w, round);//inverse round key

           state = InvMixColumns(state);//inverse mix cols


       }
       state = InvSubBytes(state);
       state = InvShiftRows(state);
       state = AddRoundKey(state, w, 0);

       for (int i = 0; i < tmp.length; i++)
           tmp[i%4*4+i/4] = state[i / 4][i%4];

       return tmp;
   }

   public static byte[] encrypt(byte[] in,byte[] key){ //for one round encryption

       Nb = 4;//no of blocks
       Nk = key.length/4; // words in a key, = 4, or 6, or 8

       Nr = Nk + 6;// corresponding number of rounds



       int lenght=0;
       byte[] padding = new byte[1];
       int i;
       lenght = 16 - in.length % 16;
       padding = new byte[lenght];/**adding extra padding bytes to fulfill 16 bytes*/ 
       padding[0] = (byte) 0x80;

       for (i = 1; i < lenght; i++)
           padding[i] = 0;

       byte[] tmp = new byte[in.length + lenght];
       byte[] bloc = new byte[16];


       w = generateSubkeys(key);

       int count = 0;

       for (i = 0; i < in.length + lenght; i++) {
           if (i > 0 && i % 16 == 0) {
               bloc = encryptBloc(bloc);
               System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);/** adding 0 as byte*/
           }
           if (i < in.length)
               bloc[i % 16] = in[i];
           else{
               bloc[i % 16] = padding[count % 16];
               count++;
           }
       }
       if(bloc.length == 16){
           bloc = encryptBloc(bloc);
           System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
       }

       return tmp;
   }

   public static byte[] decrypt(byte[] in,byte[] key){ //for one round decryption

       int i;
       byte[] tmp = new byte[in.length];
       byte[] bloc = new byte[16];


       Nb = 4;
       Nk = key.length/4;
       Nr = Nk + 6;
       w = generateSubkeys(key);


       for (i = 0; i < in.length; i++) {/**adding extra bytes to fulfill 16 bytes criteria*/
           if (i > 0 && i % 16 == 0) {
               bloc = decryptBloc(bloc);
               System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
           }
           if (i < in.length)
               bloc[i % 16] = in[i];
       }
       bloc = decryptBloc(bloc);
       System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);


       tmp = deletePadding(tmp);

       return tmp;
   }

   private static byte[] deletePadding(byte[]  input) {//to delete padding
       int count = 0;

       int i = input.length - 1;
       while (input[i] == 0) {
           count++;
           i--;
       }

       byte[] tmp = new byte[input.length - count - 1];
       System.arraycopy(input, 0, tmp, 0, tmp.length);
       return tmp;
   }

   public static byte[] encrypt(byte[] in,byte[] key,int n){// to encrypt the image for required rounds and  calculate time for encryption

       t1=System.nanoTime();//starting time(returns the current time value in nano seconds)
       Nb = 4;
       Nk = key.length/4;
       Nr=n;

       int lenght=0;
       byte[] padding = new byte[1];
       int i;
       lenght = 16 - in.length % 16;
       padding = new byte[lenght];
       padding[0] = (byte) 0x80;

       for (i = 1; i < lenght; i++)
           padding[i] = 0;

       byte[] tmp = new byte[in.length + lenght];
       byte[] bloc = new byte[16];


       w = generateSubkeys(key);

       int count = 0;

       for (i = 0; i < in.length + lenght; i++) {
           if (i > 0 && i % 16 == 0) {
               bloc = encryptBloc(bloc);
               System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
           }
           if (i < in.length)
               bloc[i % 16] = in[i];
           else{
               bloc[i % 16] = padding[count % 16];
               count++;
           }
       }


       if(bloc.length == 16){
           bloc = encryptBloc(bloc);
           System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
       }
       t2=System.nanoTime();//ending time
       return tmp;
   }

   public static byte[] decrypt(byte[] in,byte[] key,int n){//to decrypt the image for given rounds  and  calculate time for decryption


       t1=System.nanoTime();//starting time
       int i;
       byte[] tmp = new byte[in.length];
       byte[] bloc = new byte[16];


       Nb = 4;
       Nk = key.length/4;
       Nr = n;
       w = generateSubkeys(key);


       for (i = 0; i < in.length; i++) {
           if (i > 0 && i % 16 == 0) {
               bloc = decryptBloc(bloc);
               System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);
           }
           if (i < in.length)
               bloc[i % 16] = in[i];
       }
       bloc = decryptBloc(bloc);
       System.arraycopy(bloc, 0, tmp, i - 16, bloc.length);


       tmp = deletePadding(tmp);
       t2=System.nanoTime();//ending time
       return tmp;
   }

}

